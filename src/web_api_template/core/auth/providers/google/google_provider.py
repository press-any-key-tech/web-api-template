...
from .google_exception import GoogleException
from .settings import settings


class GoogleProvider(JWTAuthProvider):

    def __new__(cls):
        if not hasattr(cls, "instance"):
            cls.instance = super(GoogleProvider, cls).__new__(cls)
        return cls.instance
    ...
    def load_jwks(
        ...
        # TODO: Control errors
        jwks_uri = requests.get(settings.FIREBASE_JWKS_URL_TEMPLATE).json()
    ...
    def verify_token(self, token: JWTAuthorizationCredentials) -> bool:
        ...
        if not hmac_key_candidate:
            ...
            raise GoogleException("No public key found!")
        ...
        try:
            ...
            raise GoogleException("Error in JWTBearerManager")
